...
class kick_ball:
    ...
    def main(agent, packet):
        if kick_ball.ball_prediction != 0: ball_location = Vector3(kick_ball.ball_prediction.physics.location) #Vector2(packet.game_ball.physics.location.x, packet.game_ball.physics.location.y)
        else: ball_location = Vector3(packet.game_ball.physics.location)
        ball = agent.ball
        car = agent.car
        ...
        car_to_ball_magnitude_2d = car_to_ball_vector.get_2d().magnitude()
        car_direction = get_car_facing_vector(my_car)
        goal = get_opponents_goal(agent, packet)
        go_to = car_location
        plus = 0

        # faster or slower for ball prediction
        agent.renderer.draw_string_2d(300, 20, 2, 2, 'correction: '+str(kick_ball.correction_timer -kick_ball.prediction_timer),agent.renderer.create_color(255, 255, 255, 255))
        # ball prediction
        kick_ball.prediction_timer -= 1/60
        kick_ball.correction_timer = kick_ball.when_ball_hit(agent, packet)
        if kick_ball.prediction_timer < -0.1:
            ...
            kick_ball.ball_prediction = get_ball_prediction(agent, int(time*60))

        kick_ball.ball_prediction = get_ball_prediction(agent, int(cap_num(kick_ball.prediction_timer*60, 0, 1e9)))

        # a few variables
        ball_location = Vector3(kick_ball.ball_prediction.physics.location)
        car_to_ball_vector = Vector3(ball.pos) - Vector3(car.pos)
        car_to_ball_magnitude = car_to_ball_vector.magnitude()
        car_to_ball_magnitude_2d = car_to_ball_vector.get_2d().magnitude()
        ...
        ideal_car_to_ball_angle = (ball_location - ideal_car_location).get_2d().get_angle()
        car_to_ball_to_angle = (ball_location - car.pos).get_2d().get_angle()
        ...
        #if my_car.team == 0: print(ideal_ball_to_car_angle, ball_to_car_angle, difference_angle_car_ideal, kick_ball.get_angle_for_goal(agent, packet, ball_location))
        ...
        if Vector2(ideal_car_location.x-car_location.x, ideal_car_location.y-car_location.y).magnitude() > 1000 and not car_inline_bool and not kick_ball.in_position:
            ...
            #if agent.jumps == [] and my_car.physics.location.z < 17.1 and abs(car_direction.correction_to(ideal_car_location - car.pos)) < 0.1 and far_away_enough: double_jump(agent, packet)
            # draw ideal location
        ...
        # Else head towards the ball
            ...
            #ball_prediction = get_ball_prediction(agent, packet, round(car_to_ball_magnitude/50))
            go_to = ball_location
            plus = -difference_angle_car_ideal/(car_to_ball_magnitude/100) #-difference_angle_car_ideal/10
            ...
            if my_car.physics.location.z < 17.1 and car_inline_bool and too_slow: agent.controller_state.boost = True
    ...
    def get_angle_for_goal(agent, packet, ball_location):
        opponents_goal = get_opponents_goal(agent, packet)
    ...
    def get_ideal_car_location(agent, packet, ball_location=0):
        ...
        car = agent.car
        ball_location = Vector3(ball_location)
        car_to_ball_vector = Vector3(ball_location) - Vector3(car.pos)
        car_to_ball_magnitude = car_to_ball_vector.magnitude()
        goal = Vector3(get_opponents_goal(agent, packet))
    ...
    def predict_when_ball_hit(agent, packet):
        ...
        while loop:
            sec += 0.01+sec/12
            ball_location = get_ball_prediction(agent, round(sec/60)).physics.location
            car_duration = predict_time_needed_for_car(agent, packet, car.pos, ball_location)
            if car_duration < sec: loop = False
            if sec >= 5.5: loop = False

        return sec

    def when_ball_hit(agent, packet):
        car = agent.car
        ...
        while loop:
            sec += 0.01+sec/12
            ball_location = get_ball_prediction(agent, round(sec/60)).physics.location
            car_duration = time_needed_for_car(agent, packet, ball_location)
            if car_duration < sec: loop = False
            if sec >= 5.5: loop = False
